...
const autoSeed = async () => {
  try {
    ...
    if (productCount > 0) {
      console.log('[AUTO-SEED] Products already exist in database. Skipping seeding.');
      return;
    }

    console.log('[AUTO-SEED] Database is empty. Seeding default data...');

    // Clear everything first to prevent duplicate defaults
    ...
    console.error('[AUTO-SEED] Error seeding database:', error);
  }
};
